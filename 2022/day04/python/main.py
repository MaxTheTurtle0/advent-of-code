...
if sys.argv[1] == "1":
    ...
    for i in range(len(content)):
        pairs = [x.split("-") for x in content[i]]
        if int(pairs[0][0]) <= int(pairs[1][0]) and int(pairs[0][1]) >= int(pairs[1][0]):
            sum_pairs_full_contain += 1
        elif int(pairs[0][0]) >= int(pairs[1][0]) and int(pairs[0][0]) <= int(pairs[1][1]):
            sum_pairs_full_contain += 1
    print(f"In {sum_pairs_full_contain} assignment pairs the ranges overlap.")
else:
